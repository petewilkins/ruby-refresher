...
def get_elements_until_greater_than_five(array)
  limiter = []
  array.each {|num| num <=5 ? limiter << num : break }
  limiter
end
...
def add_together_keys_and_values(hash)
  total = 0
  hash.to_a.flatten.each {|num| total += num}
  total
end
...
def remove_capital_letters_from_string(string)
  regex = /[[:upper:]]/
  string.split('').select {|char| char unless regex.match(char)}.join
end
...
def titleize_a_string(string)
  ...
  split_string.each_with_index {|word, i| word.capitalize! if (!non_cap.include?(word) || i==0)}.join(' ')
end
